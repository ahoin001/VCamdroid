...
- **Windows: `ffmpeg does not have required feature avutil`** — The vcpkg **ffmpeg** port no longer exposes an `avutil` feature; libavutil comes with **avcodec** / **avformat**. Remove `avutil` from `windows/vcpkg.json` ffmpeg features.  
- **Windows: LNK2005 multiply defined `adb::...`** — `windows/src/adb.h` defined **non-`inline`** functions in a header included by several `.cpp` files, so each object file exported the same symbols. Mark those functions **`inline`** (or move definitions into a single `.cpp`).  
- **Windows: missing `qrcodegen.hpp`** — Add **`nayuki-qr-code-generator`** to `windows/vcpkg.json` and include `<qrcodegen.hpp>`.  
