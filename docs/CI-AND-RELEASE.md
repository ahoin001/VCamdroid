...
- **Windows: missing `qrcodegen.hpp`** — Add **`nayuki-qr-code-generator`** to `windows/vcpkg.json` and include `<qrcodegen.hpp>` (vcpkg installs it under the default include path).  
